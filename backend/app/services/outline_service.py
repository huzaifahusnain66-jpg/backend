...
Generate a comprehensive, well-structured outline."""
    ...
    def _call_with_fallback(self, messages: list, max_tokens: int) -> str:
        """Try primary model, fallback on rate limit (429)."""
        ...
        for model in models_to_try:
            try:
                ...
                if model != self._model:
                    logger.info("Used fallback model | model=%s", model)
                return response.choices[0].message.content.strip()
            except Exception as e:
                err = str(e)
                if "429" in err or "rate_limit" in err.lower():
                    logger.warning("Rate limit on %s, trying next | %s", model, err[:80])
                    last_error = e
                    continue
                raise
    ...
    def generate_outline(
        ...
        try:
            raw_json = self._call_with_fallback(
                messages=[
